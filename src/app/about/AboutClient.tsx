        ...
        <div className="section-container">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <FadeIn>
              ...
              <h1 className="text-[3.75rem] font-semibold tracking-[-0.025em] leading-[1.08] mb-6" style={s(theme.inkPrimary)}>{FOUNDER.name}</h1>
              <p className="text-[1.15rem] leading-[1.7] max-w-[44ch]" style={s(theme.inkSecondary)}>{FOUNDER.title}</p>
            </FadeIn>

            {/* Photo */}
            <FadeIn delay={0.15} className="flex justify-center lg:justify-end">
              <div className="relative w-72 h-96 rounded-[1.5rem] overflow-hidden" style={{ border: `0.5px solid ${theme.border}` }}>
                <img
                ...
                <div className="absolute bottom-0 left-0 right-0 h-24"
                  style={{ background: `linear-gradient(to top, color-mix(in srgb, ${theme.bgPrimary} 60%, transparent), transparent)` }} />
              </div>
            </FadeIn>
          </div>
        </div>
      ...
      <section className="py-28 border-b" style={{ ...bg(theme.bgPrimary), ...brd(theme.border) }}>
        <div className="section-container">
          ...
          <div className="relative max-w-[720px]">
            ...
            {TIMELINE.map((step, i) => {
              const isActive = step.period === "Current" || step.period === "Now" || step.period === "Active";
              return (
                <FadeIn key={step.company} delay={i * 0.08}>
                  <div className="relative pl-10 pb-12 last:pb-0">
                    <div className="absolute left-0 top-1 w-[23px] h-[23px] rounded-full border-2 flex items-center justify-center"
                      style={{ borderColor: isActive ? theme.accentPrimary : theme.border, background: isActive ? `color-mix(in srgb, ${theme.accentPrimary} 12%, transparent)` : theme.bgPrimary }}>
                      {isActive && <span className="w-2 h-2 rounded-full block" style={{ background: theme.accentPrimary }} />}
                    </div>
                    <div className="flex flex-wrap items-baseline gap-3 mb-1">
                      <h3 className="text-[17px] font-semibold" style={s(theme.inkPrimary)}>{step.company}</h3>
                      {isActive && <span className="text-[10px] font-medium uppercase tracking-wider" style={s(theme.accentPrimary)}>{step.period}</span>}
                    </div>
