      ...
      <section className="py-28 border-b border-[#E5E5E5] bg-white">
        <div className="section-container">
          <FadeIn>
            ...
            <span className={`text-[0.75rem] font-medium uppercase tracking-widest block mb-4 ${c.accent === "purple" ? "text-[#7C3AED]" : "text-[#2563EB]"}`}>{c.tag}</span>
            <h1 className="text-[3.75rem] font-semibold tracking-[-0.025em] text-[#111111] leading-[1.08] mb-6 max-w-[20ch]">{c.title}</h1>
            <p className="text-[1.25rem] text-[#555555] max-w-[52ch] leading-[1.7]">{c.overview}</p>
          </FadeIn>
        </div>
      </section>
            ...
            <div className="lg:col-span-2 flex flex-col gap-14">
              {[{ label:"The Challenge", body:c.challenge },{ label:"The Solution", body:c.solution },{ label:"The Result", body:c.result }].map(({ label, body }, i) => (
                <FadeIn key={label} delay={i * 0.1}>
                  <h2 className="text-[0.75rem] font-medium uppercase tracking-widest text-[#888888] mb-4">{label}</h2>
                  <p className="text-[1.125rem] text-[#555555] leading-[1.75]">{body}</p>
                </FadeIn>
              ))}
            </div>
            <div className="lg:col-span-1">
              <FadeIn delay={0.2} className="sticky top-24 flex flex-col gap-6">
                <div className="bg-white border border-[#E5E5E5] rounded-[1rem] p-8 shadow-sm">
                  <h3 className="text-[0.75rem] font-medium uppercase tracking-widest text-[#888888] mb-5">Key Points</h3>
                  <ul className="flex flex-col gap-4">
                    {c.keyPoints.map((p, i) => (
                      <li key={i} className="flex items-start gap-3">
                        <span className={`mt-1.5 w-1.5 h-1.5 rounded-full flex-shrink-0 ${c.accent === "purple" ? "bg-[#7C3AED]" : "bg-[#2563EB]"}`} />
                        <p className="text-[0.9375rem] text-[#555555] leading-[1.65]">{p}</p>
                      </li>
                    ))}
                  </ul>
                </div>
                <div className="bg-white border border-[#E5E5E5] rounded-[1rem] p-8 shadow-sm">
                  <p className="text-[0.9375rem] text-[#555555] mb-5">Interested in similar outcomes?</p>
